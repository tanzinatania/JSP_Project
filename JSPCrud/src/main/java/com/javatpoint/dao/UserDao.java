...
public static int save(User u){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("insert into register(uni_id,name,password,email,sex,campus,country,semester,batch,course) values(?,?,?,?,?,?,?,?,?,?)");
		...
		ps.setString(8,u.getSemester());
		ps.setString(9, u.getBatch());
...
public static int update(User u){
	...
	try{
		Connection con=getConnection();
		PreparedStatement ps=con.prepareStatement("update register set uni_id=?,name=?,password=?,email=?,sex=?,campus=?,country=?,semester=?,batch=?,course=? where id=?");
		ps.setString(1,u.getUni_id());
		ps.setString(2,u.getName());
		ps.setString(3,u.getPassword());
		ps.setString(4,u.getEmail());
